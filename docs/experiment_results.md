...
本文档用于集中记录第一版 QLoRA 主线实验结果，方便后续维护 README、结果表和实验复盘。
...
已完成。后续进入第 6 阶段，重点是 README、结果表和复现说明整理。
...
4. 固定 prompt 对比可以提供比单一 loss 更直观的输出观察，但仍需要结合 ROUGE-L 和人工评分表做最终结论。

结果说明：

> 固定 prompt 对比不是为了挑选好看的案例，而是为了用同一批输入观察 Base Model 和 Adapter 的差异。我保留了完整 30 条输出，并记录失败样例，避免只保留成功案例。初步看，Adapter 在结构化输出上更稳定一些，但仍会出现事实错误或概念混淆，因此后续还要结合自动指标和人工维度分析。
